...
export class RoundedCornersManager {
  ...
  enable () {
    ...
    this.connections.connect (global.display, 'restacked', () => {
      global.get_window_actors ().forEach ((actor) => {
        ...
        const shadow = this.rounded_windows?.get (actor.meta_window)?.shadow
        if (shadow) {
  ...
  should_enable_effect (win: Window): [boolean, UI.AppType] {
    ...
    // Try cache first
    const app_info = this.rounded_windows?.get (win)
    const app_type = app_info?.app_type ?? getAppType (win)
