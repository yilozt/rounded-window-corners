...
/// This file patched shell-blur-effect.c in gnome-shell project, mix with
/// rounded corners shader from mutter project (which store in shader.h), to make
/// BlurEffect with rounded corners is possible in gnome shell extensions.
/// 
/// The original effect:
/// - https://gitlab.gnome.org/GNOME/gnome-shell/-/blob/main/src/shell-blur-effect.c
///
/// Rounded Corners effect:
/// - https://gitlab.gnome.org/GNOME/mutter/-/blob/main/src/compositor/meta-background-content.c#L138

#include "patched-blur-effect.h"
...
struct _PatchedBlurEffect
{
  ...
  // Location of uniforms variants, which is defined in 'shader.h'
...
// Have to rename this type to avoid conflict with original effect
G_DEFINE_TYPE (PatchedBlurEffect, patched_blur_effect, CLUTTER_TYPE_EFFECT)
...
enum {
  ...
  PROP_BRIGHTNESS,
  PROP_MODE,
  PROP_RADIUS,  // Add new property to setup clip radius 
  N_PROPS
};
  ...
  if (G_UNLIKELY (brightness_pipeline == NULL))
    {
      ...
      // Replace original shader with rounded corners shader from 'shader.h'
...
static void
update_brightness (PatchedBlurEffect *self,
...
static gboolean
update_actor_fbo (PatchedBlurEffect *self,
                  unsigned int     width,
                  unsigned int     height,
                  float            downscale_factor)
{
...
static gboolean
update_brightness_fbo (PatchedBlurEffect *self,
                       unsigned int     width,
                       unsigned int     height,
                       float            downscale_factor)
{
...
static gboolean
update_background_fbo (PatchedBlurEffect *self,
                       unsigned int     width,
                       unsigned int     height)
{
  if (self->tex_width == width &&
...
static void
blur_effect_set_actor (ClutterActorMeta *meta,
                             ClutterActor     *actor)
{
  PatchedBlurEffect *self = PATCHED_BLUR_EFFECT (meta);
  ClutterActorMetaClass *meta_class;

  meta_class = CLUTTER_ACTOR_META_CLASS (patched_blur_effect_parent_class);
...
static void
update_actor_box (PatchedBlurEffect     *self,
                  ...
                  ClutterActorBox     *source_actor_box)
{
  ...
  switch (self->mode)
    {
    case PATCHED_BLUR_MODE_ACTOR:
      clutter_actor_get_allocation_box (self->actor, source_actor_box);
      break;

    case PATCHED_BLUR_MODE_BACKGROUND:
...
static void
add_blurred_pipeline (PatchedBlurEffect  *self,
...
create_blur_nodes (PatchedBlurEffect  *self,
                   ClutterPaintNode *node,
                   uint8_t           paint_opacity)
{
...
static void
paint_background (PatchedBlurEffect     *self,
                  ClutterPaintNode    *node,
                  ClutterPaintContext *paint_context,
                  ClutterActorBox     *source_actor_box)
{
...
static gboolean
update_framebuffers (PatchedBlurEffect     *self,
                     ClutterPaintContext *paint_context,
                     ClutterActorBox     *source_actor_box)
{
  ...
  if (self->mode == PATCHED_BLUR_MODE_BACKGROUND)
...
static void
add_actor_node (PatchedBlurEffect  *self,
                ClutterPaintNode *node,
                int               opacity)
{
...
static void
paint_actor_offscreen (PatchedBlurEffect         *self,
                       ClutterPaintNode        *node,
...
static gboolean
needs_repaint (PatchedBlurEffect         *self,
               ClutterEffectPaintFlags  flags)
{
...
static void
blur_effect_paint_node (ClutterEffect           *effect,
                              ClutterPaintNode        *node,
                              ClutterPaintContext     *paint_context,
                              ClutterEffectPaintFlags  flags)
{
  PatchedBlurEffect *self = PATCHED_BLUR_EFFECT (effect);
  uint8_t paint_opacity;
...
static void
blur_effect_finalize (GObject *object)
{
  PatchedBlurEffect *self = (PatchedBlurEffect *)object;
  ...
  G_OBJECT_CLASS (patched_blur_effect_parent_class)->finalize (object);
}

static void
blur_effect_get_property (GObject    *object,
                                guint       prop_id,
                                GValue     *value,
                                GParamSpec *pspec)
{
  PatchedBlurEffect *self = PATCHED_BLUR_EFFECT (object);
...
static void
blur_effect_set_property (GObject      *object,
                                guint         prop_id,
                                const GValue *value,
                                GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_SIGMA:
      patched_blur_effect_set_sigma (self, g_value_get_int (value));
      break;

    case PROP_BRIGHTNESS:
      patched_blur_effect_set_brightness (self, g_value_get_float (value));
      break;

    case PROP_RADIUS:
      patched_blur_effect_set_radius (self, g_value_get_float (value));
      break;

    case PROP_MODE:
      patched_blur_effect_set_mode (self, g_value_get_enum (value));
      break;
...
static void
patched_blur_effect_class_init (PatchedBlurEffectClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  ClutterActorMetaClass *meta_class = CLUTTER_ACTOR_META_CLASS (klass);
  ClutterEffectClass *effect_class = CLUTTER_EFFECT_CLASS (klass);

  object_class->finalize = blur_effect_finalize;
  object_class->get_property = blur_effect_get_property;
  object_class->set_property = blur_effect_set_property;

  meta_class->set_actor = blur_effect_set_actor;

  effect_class->paint_node = blur_effect_paint_node;
  ...
  properties[PROP_MODE] =
    g_param_spec_enum ("mode",
                       "Blur mode",
                       "Blur mode",
                       PATCHED_TYPE_BLUR_MODE,
                       PATCHED_BLUR_MODE_ACTOR,
                       G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);

  // Add property to set clip radius
...
static void
patched_blur_effect_init (PatchedBlurEffect *self)
{
  self->mode = PATCHED_BLUR_MODE_ACTOR;
  self->sigma = 0;
  self->brightness = 1.f;
  self->skip = false;
  ...
  // Query location of uniforms variants when init
...
PatchedBlurEffect *
patched_blur_effect_new (void)
{
  return g_object_new (PATCHED_TYPE_BLUR_EFFECT, NULL);
}

int
patched_blur_effect_get_sigma (PatchedBlurEffect *self)
{
  g_return_val_if_fail (PATCHED_IS_BLUR_EFFECT (self), -1);

  return self->sigma;
}

void
patched_blur_effect_set_sigma (PatchedBlurEffect *self,
                             int              sigma)
{
  g_return_if_fail (PATCHED_IS_BLUR_EFFECT (self));
...
float
patched_blur_effect_get_brightness (PatchedBlurEffect *self)
{
  g_return_val_if_fail (PATCHED_IS_BLUR_EFFECT (self), FALSE);

  return self->brightness;
}

void
patched_blur_effect_set_brightness (PatchedBlurEffect *self,
                                  float            brightness)
{
  g_return_if_fail (PATCHED_IS_BLUR_EFFECT (self));
...
float
patched_blur_effect_get_radius (PatchedBlurEffect *self)
{
  g_return_val_if_fail (PATCHED_IS_BLUR_EFFECT (self), FALSE);
...
void
patched_blur_effect_set_radius (PatchedBlurEffect *self,
                                float            radius)
{
  g_return_if_fail (PATCHED_IS_BLUR_EFFECT (self));
...
PatchedBlurMode
patched_blur_effect_get_mode (PatchedBlurEffect *self)
{
  g_return_val_if_fail (PATCHED_IS_BLUR_EFFECT (self), -1);
...
void
patched_blur_effect_set_mode (PatchedBlurEffect *self,
                            PatchedBlurMode    mode)
{
  g_return_if_fail (PATCHED_IS_BLUR_EFFECT (self));
